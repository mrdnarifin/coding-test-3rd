...
function MessageBubble({ message }: { message: Message }) {
  ...
  // Function to format text with bold and special handling for source references
  const formatTextWithBold = (text: string) => {
    ...
    return parts.map((part, idx) => {
      ...
      // Check for source references (e.g., [source 1])
      else if (part.match(/\[source \d+\]/i)) {
        const sourceNumber = part.match(/\d+/)?.[0];

        // If it's [source 1], apply special styling
        if (sourceNumber === "1") {
          return (
            <span
              key={idx}
              className='font-semibold text-blue-600 text-xs italic'
            >
              {part.toLowerCase()}
            </span>
          );
        ...
        // Regular text (non-bold, non-source)
